...
export default function StudentDashboard() {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      console.log("[v0] Student profile ID:", studentId)
      ...
      if (balanceData) {
        console.log("[v0] Balance data loaded:", balanceData)
        const remaining = balanceData.remaining_classes ?? balanceData.total_classes - balanceData.used_classes
        setRemainingClasses(remaining)
        setUsedClasses(balanceData.used_classes || 0)
      } else {
        console.log("[v0] No class balance record found, using default values")
        setRemainingClasses(0)
        setUsedClasses(0)
      }

      // Note: used_classes is now sourced from student_class_balance table
      // Admin can modify this value directly in the admin panel

      // Get today's date in YYYY-MM-DD format (Beijing timezone is UTC+8)
      const now = new Date()
      ...
      const today = beijingTime.toISOString().split("T")[0] // Format: YYYY-MM-DD
      console.log("[v0] Today's date in Beijing:", today)

      // Load today's schedules - first try with courses join, fallback to simple query
      try {
        ...
        if (schedulesError) {
          console.error("[v0] Error loading today's schedules:", schedulesError)
          setTodaySchedules([])
          setTodayClasses(0)
        } else {
          // Load course info separately if we have schedules
          if (schedulesData && schedulesData.length > 0) {
            const courseIds = [...new Set(schedulesData.map(s => s.course_id).filter(Boolean))]
        ...
        console.error("[v0] Exception loading schedules:", scheduleErr)
        setTodaySchedules([])
        setTodayClasses(0)
      }
      ...
      try {
        ...
        if (ordersError) {
          if (ordersError.message.includes("does not exist") || ordersError.message.includes("schema cache")) {
            console.log("[v0] student_orders table not found")
          } else {
            console.error("[v0] Error loading orders:", ordersError)
          }
          setOrders([])
        } else {
          console.log("[v0] Orders loaded:", ordersData?.length || 0, ordersData)
          setOrders(ordersData || [])
        }
      } catch (ordersError) {
        console.error("[v0] Exception loading orders:", ordersError)
        setOrders([])
      }
    } catch (err) {
      console.error("[v0] Exception loading dashboard data:", err)
    } finally {
      setDataLoading(false)
    }
  }, [user])
