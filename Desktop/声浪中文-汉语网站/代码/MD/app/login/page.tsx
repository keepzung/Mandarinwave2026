...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[v0] Signing out any existing session...")
        ...
        if (loginError.message.includes("Email not confirmed") || loginError.message.includes("not confirmed")) {
          ...
          throw new Error(
            language === "zh"
              ...
              : "Email not verified. Please check your inbox for the verification link. If you didn't receive it, click below to resend.",
          )
        }
        throw loginError
      }
      ...
      setError(
        err.message ||
          (language === "zh" ? "登录失败，请检查邮箱和密码" : "Login failed, please check your email and password"),
      )
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue/5 via-white to-orange/5">
      ...
      {checkingSession ? (
        <div className="flex items-center justify-center min-h-[60vh]">
          <div className="text-gray-600">{language === "zh" ? "检查登录状态..." : "Checking login status..."}</div>
        </div>
      ) : (
        <>
